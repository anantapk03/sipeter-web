...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3>Edit data akses user</h3>
        </div>
        <form action="{{route('management-features-update', ['idUser'=>$user->id, 'idAccessFeature'=>$data->id])}}" method="POST">
            @csrf
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select type="text" name="idDivisi" class="form-control" required>
                        <option value="{{$data->idDivisi}}">{{$data->divisi->namaDivisi}}</option>
